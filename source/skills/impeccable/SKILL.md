...
3. **Ejecutar impeccable teach (REQUERIDO)**: Si ninguna de las fuentes tiene contexto, DEBES ejecutar {{command_prefix}}impeccable teach AHORA antes de hacer cualquier otra cosa. NO omitas este paso. NO intentes deducir el contexto a partir del código base en su lugar.
...
Recuerda: {{model}} es capaz de realizar un trabajo creativo extraordinario. No te limites. Muestra lo que realmente se puede crear cuando se piensa fuera de lo común y se compromete plenamente con una visión distintiva.

---

## Modo Craft (Construcción)

Si esta habilidad se invoca con el argumento "craft" (por ejemplo, `{{command_prefix}}impeccable craft [descripción de la función]`), sigue el [flujo de craft](reference/craft.md). Pasa cualquier argumento adicional como la descripción de la función.

---

## Modo Teach (Aprendizaje)

Si esta habilidad se invoca con el argumento "teach" (por ejemplo, `{{command_prefix}}impeccable teach`), omite todo el trabajo de diseño anterior y en su lugar ejecuta el flujo de aprendizaje a continuación. Esta es una configuración única que recopila el contexto de diseño para el proyecto.
...
{{ask_instruction}} Enfócate solo en lo que no pudiste deducir del código base:
...
Luego {{ask_instruction}} si también desean que el Contexto de diseño se agregue a {{config_file}}. Si es así, agrégalo o actualiza la sección allí también.
...
Si esta habilidad se invoca con el argumento "extract" (por ejemplo, `{{command_prefix}}impeccable extract [objetivo]`), sigue el [flujo de extract](reference/extract.md). Pasa cualquier argumento adicional como el objetivo de extracción.
