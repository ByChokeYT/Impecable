...
STOP and call the `question` tool to clarify. Enfócate solo en lo que no pudiste deducir del código base:
...
Luego STOP and call the `question` tool to clarify. si también desean que el Contexto de diseño se agregue a AGENTS.md. Si es así, agrégalo o actualiza la sección allí también.
