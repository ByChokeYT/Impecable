...
Selecciona automáticamente las 2-3 personas más relevantes para este tipo de interfaz (usa la tabla de selección en la referencia). Si el archivo `AGENTS.md` contiene una sección `## Contexto de diseño` proveniente de `impeccable teach`, genera también 1-2 personas específicas del proyecto basándote en la información de la audiencia y de la marca.
...
**Después de presentar las conclusiones**, realiza preguntas dirigidas basándote en lo que realmente encontraste. STOP and call the `question` tool to clarify. Estas respuestas darán forma al plan de acción.
